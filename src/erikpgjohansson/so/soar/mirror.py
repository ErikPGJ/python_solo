...
PROPOSAL: erikpgjohansson.so.soar.utils.log_DST() returns string that can be
          indented by caller.
...
"""""""
    ...
    localDst = erikpgjohansson.so.soar.utils.derive_DST_from_dir(syncDir)
    ...
    #erikpgjohansson.so.soar.utils.log_DST(localDst)
    ...
    erikpgjohansson.so.soar.utils.log_DST(soarDst)
    erikpgjohansson.so.soar.utils.log_codetiming()   # DEBUG
    ...
    # local datasets.
    nRows = erikpgjohansson.so.soar.utils.nRows_DST(soarDst)
    ...
    soarDst = erikpgjohansson.so.soar.utils.index_DST(soarDst, bSoarSubset)
    print(
    ...
    #==========================================================================
    bLv     = erikpgjohansson.so.soar.utils.find_latest_versions(
        soarDst['item_id'],
        soarDst['item_version'])
    soarDst = erikpgjohansson.so.soar.utils.index_DST(soarDst, bLv)
    ...
    assert erikpgjohansson.so.soar.utils.nRows_DST(soarDst) > 0, (
    ...
    if not deleteOutsideSubset:
        ...
        localDst = erikpgjohansson.so.soar.utils.index_DST(
    ...
    soarMissingDst = erikpgjohansson.so.soar.utils.index_DST(soarDst,  bSoarMissing)
    localExcessDst = erikpgjohansson.so.soar.utils.index_DST(localDst, bLocalExcess)

    print('Online SOAR datasets that need to be downloaded:')
    erikpgjohansson.so.soar.utils.log_DST(soarMissingDst)
    print('Local datasets that need to be removed:')
    erikpgjohansson.so.soar.utils.log_DST(localExcessDst)

    # ASSERTION
    # NOTE: Deliberately doing this after logging datasets to download
    # and delete.
    nNetDatasetsToRemove = \
        erikpgjohansson.so.soar.utils.nRows_DST(localExcessDst) \
      - erikpgjohansson.so.soar.utils.nRows_DST(soarMissingDst)
    ...
    if DEBUG_DOWNLOAD_DATASETS:
        erikpgjohansson.so.soar.utils.download_latest_datasets_batch(
    ...
    nRows = erikpgjohansson.so.soar.utils.nRows_DST(localExcessDst)
...
PROPOSAL: Include file sizes.
    ...
    erikpgjohansson.so.soar.utils.assert_col_array(fileNameArray1, np.dtype('O'))
    erikpgjohansson.so.soar.utils.assert_col_array(fileNameArray2, np.dtype('O'))
    ...
    erikpgjohansson.so.soar.utils.assert_col_array(fileSizeArray1, np.dtype('int64'))
    erikpgjohansson.so.soar.utils.assert_col_array(fileSizeArray2, np.dtype('int64'))
